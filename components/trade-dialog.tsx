...
export function TradeDialog({ open, onClose, onSubmit, loading, strategies, initial }: Props) {
  ...
  const onValid = (values: FormValues) => {
    if (!values.instrument || !values.direction || !values.session ||
        !values.result || !values.grade || !strategyId) return;
    ...
    onSubmit({
      ...
      result: values.result as TradeResult,
  ...
  const canSubmit =
    !!instrument && !!direction && !!session && !!result && !!grade &&
    !!strategyId && allQuestionsAnswered && !loading;
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <ToggleGroup
                    options={["long", "short"] as TradeDirection[]}
                    value={field.value}
                    onChange={field.onChange}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="entryTime" type="time" step="600" {...register("entryTime", { required: true })} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="exitTime">Exit Time</Label>
              <Input id="exitTime" type="time" step="600" {...register("exitTime", { required: true })} />
            </div>
          </div>
            ...
            <Controller
              ...
              render={({ field }) => (
                <ToggleGroup
                  ...
                  value={field.value}
                  onChange={field.onChange}
                  renderLabel={(v) => v === "breakeven" ? "BREAK EVEN" : v.toUpperCase()}
                />
              )}
            />
          </div>
          ...
          {/* Strategy confluences — which ones applied */}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="notes"
              placeholder="Setup, mistakes, what went right..."
              rows={3}
              {...register("notes")}
            />
          </div>
